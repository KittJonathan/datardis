...
# Classic era - season 14 ----

classic_s14 <- classic_tables[[17]]

names(classic_s14) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s14 <- classic_s14 %>%
  mutate(era = "classic",
         season_number = 14,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s14_episodes <- classic_s14 %>%
  select(era:type, first_aired:duration)

classic_s14_directors <- classic_s14 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s14_writers <- classic_s14 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 15 ----

classic_s15 <- classic_tables[[18]]

names(classic_s15) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s15 <- classic_s15 %>%
  mutate(era = "classic",
         season_number = 15,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
         first_aired = as.Date(gsub(".*\\((.*)\\).*", "\\1", first_aired)),
         production_code = as.character(production_code),
         duration = c(rep(25, 16), rep(22, 4), rep(25, 6))) %>%
  group_by(story_number) %>%
  mutate(episode_number = row_number()) %>%
  ungroup() %>%
  mutate(missing_episode = 0) %>%
...
classic_s15_episodes <- classic_s15 %>%
  select(era:type, first_aired:duration)

classic_s15_directors <- classic_s15 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s15_writers <- classic_s15 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = case_when(story_number == 97 ~ "David Agnew and Graham Williams and Anthony Read",
                            TRUE  ~ writer)) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2", "writer3"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 16 ----

classic_s16 <- classic_tables[[19]]

names(classic_s16) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s16 <- classic_s16 %>%
  mutate(era = "classic",
         season_number = 16,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s16_episodes <- classic_s16 %>%
  select(era:type, first_aired:duration)

classic_s16_directors <- classic_s16 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s16_writers <- classic_s16 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 17 ----

classic_s17 <- classic_tables[[20]]

names(classic_s17) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s17 <- classic_s17 %>%
  mutate(era = "classic",
         season_number = 17,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
  ...
  select(era, season_number, serial_title, story_number, episode_number,
         episode_title, missing_episode, type, director, writer, first_aired,
         production_code, uk_viewers, rating, duration) %>%
  mutate(story_number = case_when(serial_title == "Shada" ~ "NA",
                                  TRUE ~ story_number),
         episode_title = case_when(serial_title == "Shada" ~ "NA",
                                   TRUE ~ episode_title))

classic_s17_episodes <- classic_s17 %>%
  select(era:type, first_aired:duration)

classic_s17_directors <- classic_s17 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s17_writers <- classic_s17 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = case_when(story_number == 105 ~ "David Agnew and Douglas Adams and Graham Williams and David Fisher",
                            TRUE ~ writer)) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2", "writer3", "writer4"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
  select(era:episode_number, writer = value)

rm(classic_s17)

# Classic era - season 18 ----

classic_s18 <- classic_tables[[21]]

names(classic_s18) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s18 <- classic_s18 %>%
  mutate(era = "classic",
         season_number = 18,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s18_episodes <- classic_s18 %>%
  select(era:type, first_aired:duration)

classic_s18_directors <- classic_s18 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
...
classic_s18_writers <- classic_s18 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
...
# Classic era - season 19 ----

classic_s19 <- classic_tables[[22]]

names(classic_s19) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s19 <- classic_s19 %>%
  mutate(era = "classic",
         season_number = 19,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s19_episodes <- classic_s19 %>%
  select(era:type, first_aired:duration)

classic_s19_directors <- classic_s19 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " & ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s19_writers <- classic_s19 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " & ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 22 ----

classic_s22 <- classic_tables[[26]]

names(classic_s22) <- c("story_number", "episode_number", "serial_title", "episode_title",
                         "director", "writer", "first_aired", "production_code",
                         "uk_viewers", "rating")

classic_s22 <- classic_s22 %>%
  mutate(era = "classic",
         season_number = 22,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s22_episodes <- classic_s22 %>%
  select(era:type, first_aired:duration)

classic_s22_directors <- classic_s22 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s22_writers <- classic_s22 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
classic_s23 <- classic_tables[[27]]

names(classic_s23) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s23 <- classic_s23 %>%
  mutate(era = "classic",
         season_number = 23,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s23_episodes <- classic_s23 %>%
  select(era:type, first_aired:duration)

classic_s23_directors <- classic_s23 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s23_writers <- classic_s23 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 24 ----

classic_s24 <- classic_tables[[28]]

names(classic_s24) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s24 <- classic_s24 %>%
  mutate(era = "classic",
         season_number = 24,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s24_episodes <- classic_s24 %>%
  select(era:type, first_aired:duration)

classic_s24_directors <- classic_s24 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s24_writers <- classic_s24 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 25 ----

classic_s25 <- classic_tables[[29]]

names(classic_s25) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s25 <- classic_s25 %>%
  mutate(era = "classic",
         season_number = 25,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s25_episodes <- classic_s25 %>%
  select(era:type, first_aired:duration)

classic_s25_directors <- classic_s25 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s25_writers <- classic_s25 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
...
# Classic era - season 26 ----

classic_s26 <- classic_tables[[30]]

names(classic_s26) <- c("story_number", "episode_number", "serial_title", "episode_title",
                        "director", "writer", "first_aired", "production_code",
                        "uk_viewers", "rating")

classic_s26 <- classic_s26 %>%
  mutate(era = "classic",
         season_number = 26,
         episode_title = gsub('.*"(.*)".*', "\\1", episode_title),
         type = "episode",
...
classic_s26_episodes <- classic_s26 %>%
  select(era:type, first_aired:duration)

classic_s26_directors <- classic_s26 %>%
  select(era:season_number, story_number, episode_number, director) %>%
  mutate(director = str_replace(director, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(director, c("director1", "director2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "director_name", values_drop_na = TRUE) %>%
...
classic_s26_writers <- classic_s26 %>%
  select(era:season_number, story_number, episode_number, writer) %>%
  mutate(writer = str_replace(writer, "\\s*\\([^\\)]+\\)", "")) %>%
  separate(writer, c("writer1", "writer2"), " and ") %>%
  pivot_longer(!(era:episode_number), names_to = "writer_name", values_drop_na = TRUE) %>%
