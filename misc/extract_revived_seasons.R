# Extract data for revived series
...
names(s13) <- c("story_number", "episode_number", "title",
                "director", "writer", "first_aired",
                "uk_viewers", "rating")
...
names(sp03) <- c("story_number", "episode_number", "title",
                 "director", "writer", "first_aired",
                 "uk_viewers", "rating")
...
sp03 <- sp03 %>%
  ...
  mutate(era = "revived",
         ...
         serial_title = NA,
         title = gsub('.*"(.*)".*', "\\1", title),
         director = c("Annetta Laufer", "Haolu Wang", "Jamie Magnus Stone"),
         type = "special",
         first_aired = c(as.Date("2022-01-01"), NA, NA),
         production_code = NA) %>%
  select(era, season_number, serial_title, story_number, episode_number, title, type,
         director, writer, first_aired, production_code, uk_viewers, rating)
...
episodes <- rbind(s01, s02, s03, s04, sp01, s05, s06, s07, sp02, s08, s09,
                  s10, s11, s12, s13, sp03)
episodes$episode_number[!episodes$episode_number %in% 1:13] <- NA
episodes$title[episodes$title == "TBA"] <- NA
episodes$writer[episodes$writer == "TBA"] <- NA
episodes$uk_viewers[episodes$uk_viewers == "TBD"] <- NA
episodes$rating[episodes$rating == "TBA"] <- NA
